<template>
  <AppLayout>
    <Head>
      <title>Southern Utah Headstone FAQ — Tonaquint, Hurricane, Ivins, SUNC Answers | St. George Memorials</title>
      <meta name="description" content="Answers to Washington County families' most common headstone questions — cemetery rules, temple engravings, VA markers at Southern Utah National Cemetery, snowbird ordering, hard-water maintenance, and pricing." />
      <link rel="canonical" href="https://stgeorgememorials.com/resources/faq" />
    ...
    <section class="bg-evergreen text-white py-14">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <p class="text-sage-light text-sm font-semibold uppercase tracking-widest mb-3">Frequently Asked Questions</p>
        <h1 class="font-serif text-4xl sm:text-5xl font-semibold mb-4">Headstone FAQ for Southern Utah Families</h1>
        <p class="text-white/70 text-lg max-w-3xl">The questions we hear every week from Washington, Iron, Kane, Beaver, and Garfield County families. Direct answers, no hedging.</p>
      </div>
    </section>
    ...
    <section class="py-16 bg-stone">
      <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div v-for="cat in categories" :key="cat.name" class="mb-12">
          <h2 class="font-serif text-2xl font-semibold text-evergreen mb-5">{{ cat.name }}</h2>
          <div class="space-y-3">
            <div v-for="(item, i) in cat.items" :key="`${cat.name}-${i}`" class="bg-white rounded-2xl border border-stone-dark overflow-hidden">
              <button @click="toggle(`${cat.name}-${i}`)" class="w-full flex items-center justify-between px-6 py-5 text-left">
                <span class="font-semibold text-evergreen pr-4">{{ item.q }}</span>
                <svg class="w-5 h-5 text-bronze flex-shrink-0 transition-transform" :class="{ 'rotate-180': open === `${cat.name}-${i}` }" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/></svg>
              </button>
              <div v-show="open === `${cat.name}-${i}`" class="px-6 pb-5 text-granite text-sm leading-relaxed border-t border-stone-dark pt-4" v-html="item.a"></div>
            </div>
          </div>
        </div>
        ...
        <div class="bg-evergreen text-white rounded-2xl p-8 text-center mt-10">
          ...
          <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link href="/contact" class="inline-flex items-center gap-2 bg-bronze hover:bg-bronze-dark text-white font-semibold px-8 py-3.5 rounded-lg transition-colors">Send Us a Message</Link>
            <a href="tel:+14352543122" class="inline-flex items-center gap-2 border border-white/30 hover:border-white text-white px-8 py-3.5 rounded-lg transition-colors">(435) 254-3122</a>
          </div>
        </div>
      </div>
    </section>
...
const categories = [
  {
    name: 'General',
    items: [
      { q: 'What areas do you serve?', a: 'Washington County (St. George, Hurricane, Ivins, Santa Clara, Washington City, La Verkin, Toquerville, Enterprise, and every community in between), Iron County (Cedar City, Enoch, Parowan, Paragonah, Kanarraville, Summit), Kane County (Kanab, Fredonia AZ, Orderville, Glendale, Alton, Big Water), Beaver County (Beaver, Milford, Minersville, Adamsville), and Garfield County (Panguitch, Tropic, Escalante, Boulder, Cannonville, Hatch). Southern Utah is the only market we serve.' },
      { q: 'Do I have to buy the headstone from the funeral home?', a: 'No. Under the FTC Funeral Rule, Utah families cannot be required to purchase a monument from the funeral home handling arrangements. Buying direct typically saves 30–60% on the same physical stone. See our <a href="/resources/pricing" class="text-bronze font-medium underline">pricing guide</a> for comparison numbers.' },
      { q: 'What makes ordering in Southern Utah different from ordering in Salt Lake or Provo?', a: 'Two things: climate and cemetery culture. Our desert environment (300+ sunny days, hard water, red-rock dust) makes some Northern Utah–standard granite recommendations inappropriate here. And our cemeteries — Tonaquint, Hurricane, Ivins, and the smaller pioneer sites — each have section-specific rules that Northern Utah shops don\'t know. We were built for this market specifically.' },
      { q: 'How long has St. George Memorials been operating?', a: 'We are a modern Southern Utah company built around transparent pricing, sexton-first verification, and cemetery-specific expertise. The older shops in the region (Kenworthy, Utah Monument Co) have longevity as their moat; ours is process and transparency.' },
    ],
  },
  {
    name: 'Cemetery Rules',
    items: [
      { q: 'Can I put an upright headstone at Tonaquint Cemetery?', a: 'Only in designated rows — historically rows 3 and 4 of each block. The majority of newer plots are flush-marker sections where uprights will be rejected. See our <a href="/cemeteries/tonaquint-cemetery" class="text-bronze font-medium underline">Tonaquint Cemetery guide</a> for the full section breakdown. Also note the January 16, 2026 fence-removal rule affects new decorative borders.' },
      { q: 'Does Parowan City Cemetery really only allow flat markers?', a: 'Yes. Parowan is strict flush-marker only — no upright monuments. Parowan is the "mother town of southern Utah" (settled 1851 for the Iron Mission) and the cemetery preserves a uniform lawn-level aesthetic. Ordering an upright for a Parowan lot is one of the most common costly mistakes Northern Utah shops make when they don\'t call the sexton first.' },
      { q: 'What are Hurricane City Cemetery\'s plot fees?', a: 'Currently $550 for Hurricane residents and $2,000 for non-residents, with perpetual care included. Upright monuments capped at 42 inches. Rock-pitch base recommended. Plot fees are paid directly to the city — we handle only the monument and setting.' },
      { q: 'What is the concrete apron / mow strip requirement at Ivins and Tonaquint?', a: 'A 4-to-6-inch concrete border around the monument base, poured at installation. It stabilizes the stone, protects mowing crews, and prevents hard-water runoff staining the granite face. Required at Tonaquint, Ivins, and most Washington County municipal cemeteries. Included in every quote we produce.' },
      { q: 'What changed at Tonaquint on January 16, 2026?', a: 'Effective 16 January 2026, decorative fencing and lot enclosures are prohibited. Existing fences installed before this date have grace but new installations will be removed by cemetery staff. Families designing a plot with a decorative border should have it integrated into an expanded concrete apron instead. See our <a href="/cemeteries/tonaquint-cemetery" class="text-bronze font-medium underline">Tonaquint guide</a>.' },
      { q: 'Do I need cemetery approval before ordering?', a: 'Yes. Every cemetery in Southern Utah requires monument approval before installation. Ordering without sexton verification is the most common reason families end up with rejected headstones and lost deposits with other vendors. We call the sexton on every order.' },
    ],
  },
  {
    name: 'Design & Symbols',
    items: [
      { q: 'Can I have both the St. George Temple and the Red Cliffs Temple on one headstone?', a: 'Yes — this is one of our most requested designs for Washington County couples whose memorial spans both temples. One temple appears as the primary silhouette; the other as a smaller secondary etch on the back or a companion panel. See our <a href="/headstones/st-george-temple-headstones" class="text-bronze font-medium underline">St. George Temple engraving page</a> and <a href="/headstones/red-cliffs-temple-headstones" class="text-bronze font-medium underline">Red Cliffs Temple engraving page</a>.' },
      { q: 'What is the difference between a sandblasted and a laser-etched engraving?', a: 'Sandblasted is a traditional recessed carving — the design is cut into the granite as a line drawing, then paint-filled white or contrasting color. Laser etch is a modern precision engraving on the polished granite surface — appears white against dark polished background and can reproduce photographic detail. Sandblast is more affordable and time-tested (30–50 year color life before paint refresh). Laser etch shows finer detail and is more common on jet black granite. Both hold up well in Southern Utah\'s climate.' },
      { q: 'Can I use a family photograph for a laser-etched portrait or temple?', a: 'Yes, if the resolution is high enough. Most modern phone photos taken in daylight are sufficient. We review the source before quoting. Photos taken specifically for engraving (with good lighting and focus) produce the sharpest final result.' },
      { q: 'Which granite color should I choose for Southern Utah?', a: 'For most Washington County installations we recommend Jet Black or Midnight Black polished granite. It resists UV fading, hides hard-water spotting, and provides the best contrast for laser-etched portraits and temple silhouettes. Dakota Mahogany is a strong alternative for families who want the red-rock tone. We recommend against marble in the desert — it weathers too quickly.' },
    ],
  },
  {
    name: 'Pricing',
  ...
  {
    name: 'Veterans & Southern Utah National Cemetery',
    items: [
      { q: 'What is the Southern Utah National Cemetery?', a: 'The 157th VA-operated national cemetery, dedicated 10 September 2025 in Cedar City. Located at 2181 West 1600 South, Cedar City, Utah 84720 — 45 minutes north of St. George on I-15. Free burial and government-issued marker for eligible veterans. Capacity ~13,434 gravesites, ~215 interments per year. See our <a href="/cemeteries/southern-utah-national-cemetery" class="text-bronze font-medium underline">SUNC family guide</a>.' },
      { q: 'How much does burial at SUNC cost?', a: 'Nothing for eligible veterans. Benefits include gravesite, government-issued marker, perpetual care, burial flag, Presidential Memorial Certificate, and military honors at graveside. Families are responsible for their own travel and any private services outside the VA scope.' },
      { q: 'Can I get a private custom headstone AND a VA benefit?', a: 'Yes. For veterans who died on or after November 1, 1990, the VA issues a bronze medallion that can be affixed to a private headstone in any cemetery. This gets you full custom design (temple silhouettes, laser portraits, companion monuments) with the VA recognition. Submit VA Form 40-1330M — we help with the paperwork.' },
      { q: 'Which cemetery is right for a Southern Utah veteran?', a: 'Depends on the family. SUNC (free VA marker, national-cemetery aesthetic, 45 min from St. George) is right for families wanting graveside military honors and the Arlington-style rows. A hometown cemetery like Tonaquint or Hurricane with a private stone and VA medallion is right for families wanting full custom design and closer to home. Both are valid. We help work through the decision without pushing toward the more expensive option.' },
    ],
  },
  {
  ...
  {
    name: 'Maintenance & Aftercare',
    items: [
      { q: 'How do I clean a headstone in Southern Utah?', a: 'Gentle non-ionic cleaner (available at most monument supply retailers) and a soft natural-bristle brush. Rinse with distilled water where practical. <strong>Never use vinegar, household descalers, or acidic cleaners on polished granite</strong> — they permanently etch the surface. Hard-water calcium responds well to non-ionic cleaner over multiple gentle applications. We include the first cleaning at six months on every installation.' },
      { q: 'How often should I clean the stone?', a: 'Every 18–24 months on polished jet black or midnight black granite. Every 12 months on lighter or salt-and-pepper granites. Every 6 months on marble (which we recommend against for the desert anyway). Hard-water spotting accumulates faster in Ivins and Santa Clara than in higher-elevation cemeteries.' },
      { q: 'What if my old family headstone needs restoration?', a: 'Southern Utah has more surviving pioneer-era sandstone markers than most parts of the state. We do restoration work — cleaning, resetting, re-engraving faded pioneer sandstone, and stabilizing eroding markers. Cemetery approval handled. See our <a href="/headstones/restoration" class="text-bronze font-medium underline">restoration page</a>.' },
      { q: 'What if a mower damages the headstone?', a: 'The concrete apron / mow strip we pour around every installation is specifically designed to prevent this. In the rare case of damage, cemeteries maintain grounds-keeping insurance for mower incidents — we help families navigate the claim.' },
    ],
  },
];
</script>
