<template>
  <AppLayout>
    <Head>
      <title>Southern Utah Headstone Resource Library | St. George Memorials</title>
      <meta name="description" content="Long-form guides for Southern Utah families — headstone costs in Washington County, hard-water cleaning, temple engravings, VA benefits at Southern Utah National Cemetery, snowbird ordering, and cemetery-by-cemetery rules." />
      <link rel="canonical" href="https://stgeorgememorials.com/resources" />
    </Head>

    <section class="bg-evergreen text-white py-14">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <p class="text-sage-light text-sm font-semibold uppercase tracking-widest mb-3">Resource Library</p>
        <h1 class="font-serif text-4xl sm:text-5xl font-semibold mb-4">Southern Utah Headstone Resources</h1>
        <p class="text-white/70 text-lg max-w-3xl">Written for the specific market we serve — Washington County's hard water, Southern Utah National Cemetery's brand-new VA process, the January 2026 fence-rule change at Tonaquint, and every other detail that makes ordering a headstone here different from ordering one in Salt Lake or Provo.</p>
      </div>
    </section>
    ...
    <section class="py-16 bg-stone">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <!-- Category groups -->
        <div v-for="cat in categories" :key="cat.title" class="mb-12">
          <div class="mb-5">
            <h2 class="font-serif text-2xl font-semibold text-evergreen">{{ cat.title }}</h2>
            <p class="text-sm text-granite mt-1">{{ cat.description }}</p>
          </div>
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            <Link
              ...
              :key="article.slug"
              :href="article.href"
              class="group bg-white rounded-2xl border border-stone-dark p-6 hover:border-evergreen hover:shadow-md transition-all flex flex-col"
            >
              <div class="flex items-start justify-between gap-2 mb-2">
                <p class="text-xs font-semibold uppercase tracking-widest text-bronze">{{ article.status || 'Live guide' }}</p>
                <svg class="w-4 h-4 text-evergreen/40 group-hover:text-evergreen transition-colors" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
              </div>
              <h3 class="font-serif text-lg font-semibold text-evergreen mb-2 group-hover:text-evergreen-light leading-snug">{{ article.title }}</h3>
              <p class="text-sm text-granite leading-relaxed flex-1">{{ article.desc }}</p>
              <p class="text-xs text-bronze font-medium mt-4 flex items-center gap-1">
                {{ article.status === 'Coming soon' ? 'Notify me when published' : 'Read the guide' }}
                <svg class="w-3.5 h-3.5 transition-transform group-hover:translate-x-1" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
              </p>
            </Link>
          </div>
        </div>
        ...
        <!-- CTA -->
        <div class="bg-evergreen text-white rounded-2xl p-10 text-center mt-6">
...
const categories = [
  ...
  {
    title: 'Cemeteries & Rules',
    description: 'The single biggest source of rejected headstones in Southern Utah is a mismatch between the design and the cemetery\'s section rules. These guides address that directly.',
    articles: [
      { slug: 'tonaquint-cemetery-rules-and-fees-2026', href: '/cemeteries/tonaquint-cemetery', title: 'Tonaquint Cemetery Rules, Fees, and Monument Guidelines (2026)', desc: 'Full 2026 policy digest including the January 16, 2026 fence-removal rule, section-by-section allowances, and the Veterans Memorial.' },
      { slug: 'cemetery-approval-process-washington-county', href: '#', status: 'Coming soon', title: 'How Cemetery Approval Works in Washington County', desc: 'Step-by-step: sexton contact, dimension verification, foundation specification, and installation notice — for every Washington County municipal cemetery.' },
      { slug: 'southern-utah-cemetery-comparison-hurricane-vs-ivins-vs-st-george', href: '#', status: 'Coming soon', title: 'Choosing a Cemetery in Washington County', desc: 'Hurricane vs. Ivins vs. St. George City Cemetery vs. Tonaquint — plot fees, section rules, and family fit compared.' },
      { slug: 'monument-companies-southern-utah', href: '/resources/monument-companies-southern-utah', title: 'The 2026 Guide to Monument Companies in Southern Utah', desc: 'Independent comparison of Kenworthy, Utah Monument Co (Connell\'s), Rocky Mountain, DuCrest, and online options — what each does well and what to ask before you sign.' },
    ],
  },
  {
    title: 'Design & Symbols',
    description: 'The visual language of Southern Utah headstones — temple engravings, pioneer motifs, red-rock landscape imagery.',
    articles: [
  ...
  {
    title: 'Cost & Pricing',
    description: 'Transparent price ranges for Washington County — with the local anchors (Tonaquint apron, VA offsets for veterans, snowbird shipping) that Northern Utah shops don\'t include.',
    articles: [
      { slug: 'pricing', href: '/resources/pricing', title: 'Washington County Headstone Pricing (2026)', desc: 'By product type, with the local anchors: Tonaquint concrete apron adder, VA offset for veteran families, snowbird freight surcharge, rush vs. standard timeline pricing.' },
      { slug: 'headstone-cost-washington-county-utah', href: '#', status: 'Coming soon', title: 'How Much Does a Headstone Cost in Washington County?', desc: 'Long-form 2026 price guide with local anchors — including cost differences by cemetery and by section (Tonaquint upright vs. flush, Hurricane resident vs. non-resident plot fees).' },
      { slug: 'flat-vs-upright-headstone-southern-utah', href: '#', status: 'Coming soon', title: 'Flat Marker vs. Upright Headstone in Southern Utah', desc: 'Which Washington County cemeteries require which. UV / dust performance. Cost in local dollars.' },
    ],
  },
  {
  ...
  },
  {
    ...
    description: '',
    articles: [
      { slug: 'faq', href: '/resources/faq', title: 'Headstone FAQ for Southern Utah Families', desc: 'Every question we get most often, answered directly. Timelines, cemetery rules, VA markers, snowbird ordering, maintenance, and pricing.' },
    ],
  },
];
</script>
